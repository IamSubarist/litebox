...
import { handleRegister, handleLogin } from "./store";
...
const OTPInput = ({ value, onChange, error }) => {
  ...
  return (
    <div className="space-y-3 w-full max-w-full overflow-x-hidden px-2">
      ...
      <div className="flex gap-1 sm:gap-2 justify-center flex-wrap">
        {otp.map((digit, index) => (
          <input
            ...
            onPaste={handlePaste}
            className={`form-control form-label w-10 h-10 sm:w-12 sm:h-12 text-center text-lg sm:text-xl font-semibold border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary-500 flex-shrink-0 ${
              error
                ? "border-danger-500"
...
const RegForm = () => {
  ...
  // Сброс ошибки чекбокса через 1.5 секунды
  useEffect(() => {
    if (error) {
      setTimeout(() => {
        setError(false);
  ...
  const onEmailSubmit = async (data) => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/auth/confirm_email?recovery=false`,
        {
          login: data.email,
        },
        {
  ...
  const handleOTPContinue = async () => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/auth/confirm_email?recovery=false&code=${otp}`,
        {
          login: email,
        },
        {
  ...
  const onPasswordSubmit = async (data) => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/auth/create_user?guard_hash=${hash}`,
        {
          login: email,
      ...
      <div className="flex items-center justify-center gap-2 mb-6 flex-wrap">
        {[1, 2, 3].map((stepNumber) => (
          <React.Fragment key={stepNumber}>
            ...
            {stepNumber < 3 && (
              <div
                className={`h-1 w-8 sm:w-12 flex-shrink-0 ${
                  step > stepNumber
                    ? "bg-primary-500"
        ...
        <div className="space-y-5 w-full max-w-full overflow-x-hidden">
          ...
          <div className="flex gap-2 sm:gap-3 flex-wrap">
            <button
              ...
              className="btn btn-outline-dark flex-1 min-w-0"
              disabled={loading}
            >
              Back
            </button>
            <button
              ...
              className="btn btn-dark flex-1 min-w-0 disabled:opacity-50 disabled:cursor-not-allowed"
            >
